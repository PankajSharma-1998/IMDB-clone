...
if(list.length === 0){
    ...
    (async function(){
        ...
        const key="437ff009500a18a6e04fbe55351aacbf";
         const popular_url=`https://api.themoviedb.org/3/${type}/popular?api_key=${key}&language=en-US&page=1`;
            
         const res= await axios.get(popular_url);
         setlist([...res.data.results].slice(0,11));
